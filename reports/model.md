...
month has been read 7 time(s) so far; each read and its purpose is
...
One transaction at a time (median / 99th percentile over 500 validation rows, on the development laptop): score 0.21 / 0.28 ms; score with exact TreeSHAP reasons 5.4 / 5.7 ms. TreeSHAP costs trees × leaves × depth², which is why the trees are limited to depth 8 (DECISIONS.md).
...
| Logistic regression | 0.196 | 0.822 | 2.6% / 9.6% / 14.0% / 38.9% | 15.1% | 0.0297 | 0.0040 |
| LightGBM | 0.616 | 0.926 | 29.0% / 47.0% / 54.0% / 71.8% | 60.9% | 0.0190 | 0.0018 |
...
- **Logistic regression** — the explainable features without the masked card and address codes; signed log transform, standardised, one-hot categories. C = 1.0, class weight = None, chosen from 8 settings by validation PR-AUC. Calibration: none.
- **LightGBM** — all 452 features (history aggregates, readable fields and Vesta's anonymised columns). num_leaves = 63, min_child_samples = 50, scale_pos_weight = 1.0, 1559 trees (early stopping on validation PR-AUC), chosen from 8 settings. Calibration: platt.
...
|---|---|---|---|---|
| Logistic regression | 0.02971 | 0.02976 | 0.02977 | none |
| LightGBM | 0.01962 | 0.01910 | 0.01911 | platt |
